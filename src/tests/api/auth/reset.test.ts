...
describe('POST /api/auth/reset-password', () => {
  ...
  (findUserByResetToken as jest.Mock).mockResolvedValue({ email: 'user@example.com' });

  const res = await POST(makeReq({ token: 'valid', password: 'newpassword' }));
  ...
  expect(updateUserPassword).toHaveBeenCalledWith('user@example.com', 'newpassword');
  });
...
describe('resetPasswordSchema validation', () => {
  ...
  it('should fail if password is less than 8 characters', () => {
    const { error } = resetPasswordSchema.validate({ token: 'validtoken', password: 'Ab1!' });
    expect(error).toBeDefined();
  ...
  it('should fail if password does not meet complexity', () => {
    const { error } = resetPasswordSchema.validate({ token: 'validtoken', password: 'abcdefgh' });
    expect(error).toBeDefined();
  ...
  const { error } = resetPasswordSchema.validate({ token: 'validtoken', password: '' });
  expect(error).toBeDefined();
  expect(error?.details[0].message).toBe('Password is required');
  });
  
  it('should fail if token is empty string', () => {
  const { error } = resetPasswordSchema.validate({ token: '', password: 'Abcd1234!' });
  expect(error).toBeDefined();
  expect(error?.details[0].message).toBe('Reset token is required');
  });
